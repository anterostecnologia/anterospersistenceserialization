...
public class AnterosProxySerializer extends JsonSerializer<AnterosProxyObject> {
	...
	protected Object findProxied(AnterosProxyObject proxy) throws Exception {
		MethodHandler handler = ((ProxyObject)proxy).getHandler();
		if (!(handler instanceof LazyLoadInterceptor))
			return null;

		if (!_forceLazyLoading && !((LazyLoadInterceptor) (handler)).isInitialized()) {
			return null;
		}
		return ((LazyLoadInterceptor) (handler)).initializeAndReturnObject();
	}
